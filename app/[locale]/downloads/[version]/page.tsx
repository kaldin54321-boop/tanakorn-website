            ...
            {release.file_path || (release as any).external_url ? (
              <>
                <div>
                  ...
                  <p>APK • {release.architecture} • {formatFileSize(release.file_size, dict)}{(release as any).external_url ? ` • ${dict.common.external}` : ""}</p>
                  {(release as any).external_url && (
                    <p style={{ fontSize:"11px", color:"var(--muted)", wordBreak:"break-all", marginTop:"6px"}}>{dict.common.external}: {(release as any).external_url}</p>
                  )}
                </div>
